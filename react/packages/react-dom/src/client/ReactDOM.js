...
function ReactRoot(
  ...
  this._internalRoot = root;  //DOMRenderer.createContainer创建root，然后挂载到_internalRoot上面
}
...
function legacyCreateRootFromDOMContainer(
  ...
  if (!shouldHydrate) {  //非服务端渲染
    let warned = false;
    let rootSibling;
    while ((rootSibling = container.lastChild)) {  //通过container.lastchild循环来清除container的所有内容，因为我们的属于首次渲染，container里边不包含任何元素
      if (__DEV__) {
        if (
          !warned &&
      ...
      container.removeChild(rootSibling);  //把container下的子节点都删掉
    }
  }
  if (__DEV__) {
  ...
  return new ReactRoot(container, isConcurrent, shouldHydrate);  // 通过ReactRoot来创建一个root
}
...
function legacyRenderSubtreeIntoContainer(
  ...
  children: ReactNodeList,
  container: DOMContainer,
  forceHydrate: boolean,  //服务端渲染：复用节点
  callback: ?Function,
) {
  ...
  let root: Root = (container._reactRootContainer: any);  //正常传入的container不会携带_reactRootContainer属性，所以第一次渲染的时候只关注if (!root)
  if (!root) {
    // Initial mount
    root = container._reactRootContainer = legacyCreateRootFromDOMContainer( // 创建根节点
      container,
      forceHydrate,
    );
    ...
    DOMRenderer.unbatchedUpdates(() => {
      if (parentComponent != null) {
        ...
        root.render(children, callback);  //调用了render方法
      }
    });
  } else {
    if (typeof callback === 'function') {
